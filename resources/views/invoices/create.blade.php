...
@push('styles')
    ...
    <style>
        /* محاذاة شكل محرر Quill مع باقي حقول الفورم (.dash-field). القيم اللونية
           هنا منسوخة حرفياً من tailwind.config.js (subtle/muted) لأن هذا
...
<div class="max-w-4xl mx-auto space-y-5" x-data="invoiceForm()">
    ...
    <form method="POST" action="{{ route('invoices.store') }}" class="space-y-5" @submit="syncEditors()">
        @csrf
        ...
        <x-card-section title="ملاحظات وشروط">
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="hidden" name="terms" value="{{ old('terms') }}">
                </div>
            </div>
        </x-card-section>
...
function invoiceForm() {
    return {
        ...
        initEditor(refName, hiddenInputSelector, placeholder) {
            ...
            // تعبئة القيمة القديمة (عند فشل تحقق الفورم، أو نص قديم غير منسّق
            // من فواتير أُنشئت قبل هذه الميزة — لا يحتوي أي وسم HTML، فقط
            // أسطر نصية عادية؛ نحوّل فواصل الأسطر إلى <br> يدوياً قبل التعبئة
            // حتى لا تختفي بصرياً عند تحويلها لـHTML خام).
