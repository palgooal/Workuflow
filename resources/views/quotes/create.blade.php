...
<div class="max-w-4xl mx-auto space-y-5" x-data="quoteForm()">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-ink tracking-tight">إنشاء عرض سعر</h1>
            <p class="text-sm text-slate-500">أدخل بيانات العرض والخدمات المقدَّمة</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('quotes.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="3" placeholder="أي ملاحظات تودّ إيصالها للعميل..."
                              class="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200
                                     focus:outline-none focus:ring-2 focus:ring-accent/40">{{ old('notes') }}</textarea>
                </div>
                <div>
                    ...
                    <textarea name="terms" rows="3" placeholder="شروط الدفع، ضمانات، استثناءات..."
                              class="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200
                                     focus:outline-none focus:ring-2 focus:ring-accent/40">{{ old('terms') }}</textarea>
                </div>
            </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('quotes.index') }}"
               class="px-5 py-2.5 text-sm text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition">
                إلغاء
            </a>
            <button type="submit"
