    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5"
        ...
        <div class="flex items-start justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">{{ $quote->number }} — {{ $quote->client->name }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('quotes.convert', $quote->ulid) }}" class="space-y-4">
            ...
            <div class="bg-teal-50 border border-teal-200 rounded-xl p-4 space-y-1 text-sm">
                <div class="flex justify-between text-gray-600">
                    <span>الإجمالي</span>
                    <span class="font-semibold text-gray-900">{{ number_format($quote->total, 2) }} {{ $quote->currency }}</span>
                </div>
                <div class="flex justify-between text-gray-600">
                ...
                <div class="flex justify-between text-gray-600">
                    ...
                    <span class="text-indigo-600">{{ $quote->project->name }}</span>
                </div>
                @endif
            </div>
