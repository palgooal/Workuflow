<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                <div class="flex items-center gap-2 text-sm text-gray-500 mb-1">
                    ...
                    <span>/</span>
                    <span class="text-gray-700 font-medium">إدارة الوسوم</span>
                </div>
                <h2 class="text-xl font-bold text-gray-900">🏷️ إدارة الوسوم</h2>
            </div>
                ...
                <div x-show="showCreateForm" x-cloak x-transition class="px-6 py-5">
                    <form @submit.prevent="createTag()" class="flex flex-wrap gap-4 items-end">
                        ...
                        <div class="flex-1 min-w-48">
                            <label class="block text-xs font-medium text-gray-600 mb-1">اسم الوسم *</label>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-gray-500 font-mono" x-text="newTag.color"></span>
                            </div>
                        </div>

                        {{-- الأيقونة --}}
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">أيقونة (Emoji)</label>
                            <input x-model="newTag.icon"
                                   type="text"
                                   placeholder="🏷️"
                                   maxlength="4"
                                   class="w-20 border border-gray-300 rounded-lg px-3 py-2 text-sm text-center focus:ring-2 focus:ring-indigo-500">
                        </div>

                        {{-- معاينة --}}
                        <div>
                            <label class="block text-xs font-medium text-gray-600 mb-1">معاينة</label>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">يمكنك تعديلها وحذفها وإعادة ترتيبها بالسحب</p>
                    </div>
                ...
                @else
                    <div id="custom-tags-list" class="divide-y divide-gray-50">
                        @foreach($customTags as $tag)
                            <div class="tag-row flex items-center gap-4 px-6 py-4 hover:bg-gray-50 transition group"
                                 data-id="{{ $tag->id }}"
                                ...
                                <div x-show="!editing" class="flex items-center gap-3 flex-1 min-w-0">
                                    ...
                                    @if($tag->clients_count > 0)
                                        <a href="{{ route('clients.index', ['tag' => $tag->id]) }}"
                                           class="text-xs text-indigo-500 hover:text-indigo-700 opacity-0 group-hover:opacity-100 transition">
                                            عرض العملاء ←
                                        </a>
                                    @endif
                                </div>
                                ...
                                <div x-show="editing" x-cloak class="flex items-end gap-3 flex-1 flex-wrap">
                                    <div>
                                        ...
                                        <input x-model="editName" type="text" maxlength="50"
                                               class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-36 focus:ring-2 focus:ring-indigo-400">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-500 mb-1">اللون</label>
                                        <input x-model="editColor" type="color"
                                               class="w-9 h-9 rounded-lg border border-gray-300 p-0.5 cursor-pointer">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-500 mb-1">أيقونة</label>
                                        <input x-model="editIcon" type="text" maxlength="4" placeholder="🏷️"
                                               class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm w-16 text-center focus:ring-2 focus:ring-indigo-400">
                                    </div>
                                    {{-- معاينة --}}
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    ...
                                    <template x-if="!editing">
                                        <button @click="startEdit()"
                                                class="text-xs text-gray-400 hover:text-indigo-600 opacity-0 group-hover:opacity-100 transition px-2 py-1 rounded hover:bg-indigo-50">
                                            ✏️ تعديل
                                        </button>
                                    </template>
                                    ...
                                    <template x-if="editing">
                                        <div class="flex gap-2">
                                            <button @click="saveEdit()"
                                                    :disabled="saving"
                                                    class="text-xs text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1.5 rounded-lg disabled:opacity-50 transition">
                                                <span x-show="!saving">💾 حفظ</span>
                                                <span x-show="saving">…</span>
                                            </button>
                                            <button @click="cancelEdit()"
                                                    class="text-xs text-gray-500 bg-gray-100 hover:bg-gray-200 px-3 py-1.5 rounded-lg transition">
                                                إلغاء
                                            </button>
                                        </div>
                                    </template>
                ...
                @if($customTags->count() > 1)
                    <div class="px-6 py-3 border-t border-gray-50 bg-gray-50 rounded-b-xl">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">وسوم مدمجة ومشتركة بين جميع المستخدمين — للعرض فقط</p>
                    </div>
                    <span class="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded-full">
    ...
    <div id="delete-modal"
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6">
            <div class="text-center">
                <div class="text-4xl mb-3">🗑️</div>
                <h3 class="text-lg font-bold text-gray-900 mb-1">حذف الوسم</h3>
                <p class="text-sm text-gray-600 mb-1">هل أنت متأكد من حذف وسم</p>
                <p class="text-sm font-bold text-gray-800 mb-4" x-text='"«" + tagName + "»"'></p>
    ...
    <div x-data="{ show: false, msg: '', error: false }"
         ...
         :class="error ? 'bg-red-600' : 'bg-gray-900'">
