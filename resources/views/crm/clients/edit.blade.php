...
<div class="max-w-3xl mx-auto space-y-5">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-xl font-bold text-ink tracking-tight">تعديل: {{ $client->name }}</h1>
            <p class="text-sm text-slate-500">تحديث بيانات العميل</p>
        </div>
    </div>
    ...
    <form id="edit-form" method="POST" action="{{ route('clients.update', $client->public_id) }}" class="space-y-5">
        ...
        <div class="dash-card p-5 space-y-4">
            <h2 class="text-sm font-semibold text-slate-700">البيانات الأساسية</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    <label class="block text-sm font-semibold text-ink mb-1">الاسم الكامل <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-ink mb-1">الهاتف</label>
                ...
                <div>
                    <label class="block text-sm font-semibold text-ink mb-1">الشركة</label>
                    <input type="text" name="company" value="{{ old('company', $client->company) }}"
                           class="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-accent/40 outline-none">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-ink mb-1">المنصب</label>
        ...
        <div class="dash-card p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-ink mb-1">الحالة</label>
                    <select name="status"
        ...
        <div class="dash-card p-5 space-y-3">
            <h2 class="text-sm font-semibold text-slate-700">الوسوم</h2>
        ...
        {{-- معلومات تكميلية --}}
        <div class="dash-card p-5 space-y-4">
            <h2 class="text-sm font-semibold text-slate-700">معلومات تكميلية</h2>
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full px-4 py-2.5 text-sm border border-slate-200 rounded-xl focus:ring-2 focus:ring-accent/40 outline-none resize-none">{{ old('notes', $client->notes) }}</textarea>
            </div>
        </div>
