<div class="px-5 py-4 hover:bg-gray-50/70 transition group">
        ...
        <div class="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    <p class="text-sm font-semibold text-gray-900">{{ $debt->party_name }}</p>
                ...
                <div class="mt-1 flex items-center gap-3 text-xs text-gray-500">
                    <span>الإجمالي: <strong class="text-gray-700">{{ number_format($debt->amount, 2) }} {{ $debt->currency }}</strong></span>
                    @if(!$debt->isPaid())
                        <span class="text-gray-300">|</span>
                        <span>المتبقي: <strong class="text-red-600">{{ number_format($debt->remaining_amount, 2) }} {{ $debt->currency }}</strong></span>
                    @endif
                    @if($debt->due_date)
                        <span class="text-gray-300">|</span>
                ...
                @if(!$debt->isPaid())
                    ...
                    <div class="mt-2.5">
                        <div class="flex justify-between items-center mb-1">
                            <span class="text-xs text-gray-400">تقدم السداد</span>
                ...
                {{-- Notes --}}
                @if($debt->notes)
                    <p class="mt-1.5 text-xs text-gray-400 truncate">📝 {{ $debt->notes }}</p>
                @endif
            </div>
        </div>
