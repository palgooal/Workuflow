...
<div class="space-y-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <h1 class="text-xl font-bold text-ink tracking-tight">{{ $project->name }}</h1>
            ...
            <div class="flex items-center gap-2 shrink-0">
                <a href="{{ route('projects.edit', $project) }}"
                   class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium
                          text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-btn transition-colors">
    ...
    {{-- عملات متعددة: جدول --}}
    <div class="dash-card overflow-hidden">
        <div class="grid grid-cols-5 dash-th px-5 py-2.5 border-b border-subtle bg-slate-50/70">
            <div>العملة</div>
        ...
        <div class="grid grid-cols-5 items-center px-5 py-3 {{ !$loop->last ? 'border-b border-slate-50' : '' }}">
            <div class="flex items-center gap-1.5">
                <span class="text-sm font-semibold text-slate-700">{{ $cur }}</span>
                @if($cur === $projCur)
                <span class="text-xs bg-brand-100 text-brand px-1.5 py-0.5 rounded font-medium">أساسي</span>
                @endif
            </div>
            ...
            <div class="text-center font-bold text-sm {{ $vals['net'] >= 0 ? 'text-brand-600' : 'text-red-600' }}">
        ...
        <div class="dash-card p-5">
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-semibold text-slate-900">قيمة العقد</span>
                    <div class="relative" x-data="{ show: false }">
                        <button type="button" @mouseenter="show=true" @mouseleave="show=false"
                                class="w-4 h-4 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center
                                       justify-center text-slate-500 text-xs font-bold cursor-help">?</button>
                        <div x-show="show" x-cloak
                             class="absolute bottom-full mb-2 right-0 z-50 w-60 p-3 bg-slate-900 text-white
            ...
            <div class="flex justify-between text-xs text-slate-500">
                <span>مُستلم: <strong class="text-slate-800">{{ number_format($summary['income'], 2) }}</strong></span>
                <span>العقد: <strong class="text-slate-800">{{ number_format($summary['contract_value'], 2) }}</strong></span>
            </div>
        ...
        <div class="dash-card p-5
            ...
            <div class="flex items-center justify-between mb-3">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-semibold text-slate-900">ميزانية التكاليف</span>
                    <div class="relative" x-data="{ show: false }">
                        <button type="button" @mouseenter="show=true" @mouseleave="show=false"
                                class="w-4 h-4 rounded-full bg-slate-200 hover:bg-slate-300 flex items-center
                                       justify-center text-slate-500 text-xs font-bold cursor-help">?</button>
            ...
            <div class="flex justify-between text-xs text-slate-500">
                <span>مُنفَق: <strong class="text-slate-800">{{ number_format($summary['expenses'], 2) }}</strong></span>
                <span>الميزانية: <strong class="text-slate-800">{{ number_format($summary['expense_budget'], 2) }}</strong></span>
            </div>
    ...
    <div class="dash-card p-5">
        ...
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="text-xs text-slate-500">
                إجمالي التكاليف:
                <strong class="text-slate-700">{{ number_format($summary['total_members_cost'], 2) }} {{ $project->currency }}</strong>
            </div>
            @endif
        </div>
        ...
        <div class="space-y-3">
            ...
            @endphp
            <div class="rounded-xl border {{ $marginClasses['border'] }} bg-slate-50 overflow-hidden">

                {{-- Service Row --}}
                <div class="flex items-center justify-between px-4 py-3">
                    <div class="min-w-0">
                        <p class="text-sm font-semibold text-slate-900 truncate">{{ $svcMargin['name'] }}</p>
                        <div class="flex items-center gap-3 mt-0.5 text-xs text-slate-500">
                            <span>إيراد: <strong class="text-slate-700">{{ number_format($svcMargin['revenue'], 2) }}</strong></span>
                            @if($svcMargin['members_cost'] > 0)
                            <span>تكلفة: <strong class="text-slate-700">{{ number_format($svcMargin['members_cost'], 2) }}</strong></span>
                            @endif
                        </div>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0">
                        ...
                        @endif
                        <span class="text-sm font-bold {{ $svcMargin['is_loss'] ? 'text-red-600' : 'text-slate-800' }}">
                ...
                <div class="px-4 py-2.5 space-y-1.5 bg-white border-t border-slate-100">
                    @foreach($svcMargin['members'] as $memberData)
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <div class="w-6 h-6 rounded-md bg-brand-100 flex items-center justify-center
                                        text-brand font-bold text-xs flex-shrink-0">
                                {{ mb_substr($memberData['name'], 0, 1) }}
                            </div>
                            <span class="text-xs text-slate-700 font-medium">{{ $memberData['name'] }}</span>
                        </div>
                        <div class="flex items-center gap-2">
                            @if($memberData['team_cost'] > 0)
                            <span class="text-xs font-semibold {{ $memberData['team_cost_paid'] ? 'text-green-600' : 'text-slate-700' }}">
        ...
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <h2 class="font-semibold text-slate-900 flex items-center gap-2">
                ...
                @if($projectQuotes->count() > 0)
                    <span class="px-2 py-0.5 text-xs bg-brand-100 text-brand-600 rounded-full font-medium">
                        {{ $projectQuotes->count() }}
                    </span>
                @endif
            </h2>
            <a href="{{ route('quotes.create') }}?project_id={{ $project->id }}&client_id={{ $project->client_id }}"
               class="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm text-brand
                      border border-brand/30 bg-brand-50 rounded-xl hover:bg-brand-100 transition">
                + إنشاء عرض
            </a>
        </div>
                ...
                <a href="{{ route('quotes.show', $q->ulid) }}"
                   class="flex items-center gap-4 px-5 py-3.5 hover:bg-slate-50 transition">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2">
                            <span class="text-sm font-medium text-slate-900">{{ $q->number }}</span>
                            @if($q->title)
                                <span class="text-xs text-slate-400 truncate">— {{ $q->title }}</span>
                            @endif
                        </div>
                        <p class="text-xs text-slate-400 mt-0.5">{{ $q->issue_date->format('d/m/Y') }}</p>
                    </div>
    ...
    <div class="dash-card">
        <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
            <h2 class="font-semibold text-slate-900">آخر المعاملات</h2>
            <a href="{{ route('transactions.index') }}?project={{ $project->id }}"
               class="text-sm text-brand hover:text-brand-600">
