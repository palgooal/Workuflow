...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <x-stats-card
            title="إجمالي الدخل"
            :value="number_format($portfolio['total_income'], 2)"
            color="green"
            prefix=""
        >
            <x-slot name="icon">
        ...
        <x-stats-card
            ...
            :value="number_format($portfolio['total_expenses'], 2)"
            color="red"
        >
            <x-slot name="icon">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <x-stats-card
            title="صافي الربح"
            :value="number_format(abs($portfolio['total_net']), 2)"
            :color="$portfolio['total_net'] >= 0 ? 'green' : 'red'"
            :prefix="$portfolio['total_net'] >= 0 ? '+' : '-'"
        >
            <x-slot name="icon">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
        ...
        <x-stats-card
            ...
            :value="$portfolio['active_count'] . ' / ' . $portfolio['projects_count']"
            color="indigo"
        >
            <x-slot name="icon">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
