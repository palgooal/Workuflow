...
## العملات المدعومة للدفع — v1.7.2

عملة الفاتورة (`invoice.currency`) يمكن أن تكون **أي عملة** من القائمة الكاملة المدعومة في `App\Support\Helpers\Currency` (25 عملة) — لا قيد على ذلك عند إنشاء الفاتورة أو تعديلها أو عرضها أو إرسالها أو طباعتها.

**لكن الدفع الإلكتروني عبر بوابة Togo مدعوم حالياً فقط لعملتين:**
...
القائمة معرَّفة في `TogoPaymentService::supportedInvoiceCurrencies()`، وتُفحَص عبر `TogoPaymentService::isInvoiceCurrencySupported(string $currency)` (فحص غير حسّاس لحالة الأحرف).

### ماذا يحدث لفاتورة بعملة غير مدعومة؟

- **العرض (`InvoicePaymentController@show`):** صفحة الفاتورة العامة (`/pay/invoice/{ulid}`) تُعرَض كالمعتاد بكل تفاصيلها. يظهر تنبيه واضح: *"هذه العملة غير مدعومة للدفع الإلكتروني حالياً. العملات المدعومة: ILS, USD."*، ويختفي زر **"ادفع الآن"**.
- **بدء الدفع (`InvoicePaymentController@checkout`):** فحص صارم إضافي قبل أي إنشاء لسجل `payment_collections` أو استدعاء لـ Togo API — لو حاول أحد الوصول للمسار مباشرة (تجاوز الواجهة)، يُعاد توجيهه فوراً إلى صفحة الفاتورة مع رسالة الخطأ نفسها، ولا يُنشأ أي `PaymentCollection`.
- **لا تأثير على أي شيء آخر:** `invoice.currency`، `invoice.total`، منطق `markPaid`، وجدول `payment_collections` كلها بلا أي تغيير. الفاتورة تبقى قابلة للإرسال والطباعة عبر القنوات الأخرى (PDF، واتساب، إيميل) بغض النظر عن هذا القيد — القيد خاص فقط بمسار الدفع الإلكتروني العام.

### مستقبلاً: "الدفع بما يعادلها بالشيكل"

فكرة مستقبلية (غير مُنفَّذة بعد) هي السماح بدفع فاتورة بعملة غير مدعومة عبر تحويلها لِما يعادلها بالشيكل عند الدفع. **لم تُنفَّذ الآن لغياب مصدر سعر صرف رسمي موثوق** — لا نفترض سعر صرف اعتباطياً (نفس المبدأ المتّبع في "عملة الفاتورة مقابل عملة التسوية" أعلاه). تحتاج هذه الميزة أولاً مصدر سعر صرف رسمي قبل أي تنفيذ.
