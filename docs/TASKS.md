...
> آخر تحديث: 9 يونيو 2026 — Phase 26 (Communication Layer) مُخطَّط ✅
...
| البيان | القيمة |
|--------|--------|
| إجمالي المراحل | 21 مرحلة + Marketing + Admin+ + UX |
...
Phase 26 → Communication Layer (Email + Notifications + WhatsApp)       📋 مُخطَّط
  ↳ Sprint 1 — Email Foundation                                         📋 مُخطَّط
  ↳ Sprint 2 — Business Emails                                          📋 مُخطَّط
  ↳ Sprint 3 — Notification Center                                      📋 مُخطَّط
  ↳ Sprint 4 — WhatsApp Quick Actions                                   📋 مُخطَّط
Phase 27 → WhatsApp Business Center (Full API)                          🔮 مستقبلي
Phase 28 → Cash Flow Forecast Engine                                    🔮 مستقبلي
Phase 29 → AI Insights Engine Phase 1                                   🔮 مستقبلي
Phase 30 → Payment Gateway Integration                                  🔮 مستقبلي
```
...
> تدقيق نظام البريد: `docs/EMAIL-SYSTEM-AUDIT.md`  
> الهدف: إصلاح البنية التحتية للتواصل قبل الإطلاق التجاري الأول  
> الجهد الكلي: ~61 ساعة | 4 sprints | 4 أسابيع

---

### Sprint 1 — Email Foundation (أساس البريد الإلكتروني)

**الهدف:** تصفير المشاكل الجوهرية — جميع الإيميلات في Queue، جميعها تستخدم نظام القوالب، التحقق من البريد مُفعَّل.

#### 26.1 — إصلاح البنية التحتية
- [ ] إعادة تفعيل `MustVerifyEmail` في `app/Models/User.php`
- [ ] إلغاء تعليق `use Illuminate\Contracts\Auth\MustVerifyEmail;` (السطر 7)
- [ ] إضافة `middleware('verified')` لمجموعات Routes المحمية في `routes/web.php`
- [ ] Migration بسيطة: تعيين `email_verified_at = now()` لجميع المستخدمين الحاليين (لتجنب إغلاق الحسابات)

#### 26.2 — Queue للبريد الإلكتروني
- [ ] إنشاء `app/Jobs/SendInvoiceEmailJob.php` — ShouldQueue, queue:'emails', tries:3, timeout:90, backoff:[30,120,300], failed()
- [ ] تحديث `InvoiceController::sendEmail()` — dispatch `SendInvoiceEmailJob` بدلاً من `Mail::to()->send()` المباشر
- [ ] إضافة queue runner للـ Scheduler في `routes/console.php` (cPanel-compatible)

#### 26.3 — توحيد نظام القوالب
- [ ] تحديث `app/Mail/InvoiceReminderMail.php` — استبدال الـ HTML الثابت بـ `EmailTemplate::render('invoice_reminder')`
- [ ] إنشاء `database/seeders/EmailTemplateSeeder.php` يحتوي على 8 قوالب جديدة:
  - `invoice_reminder` — تذكير فاتورة مستحقة
  - `invoice_paid` — تأكيد استلام الدفعة
  - `quote_send` — إرسال عرض السعر للعميل
  - `quote_accepted` — تأكيد قبول عرض السعر (للمستخدم)
  - `quote_rejected` — إشعار رفض عرض السعر (للمستخدم)
  - `debt_due_soon` — تذكير دين مستحق
  - `debt_overdue` — تنبيه دين متأخر
  - `follow_up_reminder` — تذكير متابعة عميل

#### 26.4 — Testing Sprint 1
- [ ] Unit: `SendInvoiceEmailJobTest` — dispatch, queue, retry, failed()
- [ ] Unit: `InvoiceReminderMailTemplateTest` — uses EmailTemplate, fallback body
- [ ] Feature: `EmailTemplateSeederTest` — all 8 keys inserted correctly

---

### Sprint 2 — Business Emails (إيميلات الأعمال)

**الهدف:** تغطية كامل دورة حياة عروض الأسعار والفواتير بالبريد. إيصال تنبيهات الديون والمتابعات للمستخدمين حتى وهم غير متصلين.

#### 26.5 — QuoteMail
- [ ] إنشاء `app/Mail/QuoteMail.php` — EmailTemplate('quote_send')، Signed URL 30 يوم، ShouldQueue
- [ ] إنشاء `app/Jobs/SendQuoteEmailJob.php` — ShouldQueue, queue:'emails', tries:3, failed()
- [ ] تحديث `QuoteController` — dispatch `SendQuoteEmailJob` عند إرسال عرض السعر للعميل

#### 26.6 — Quote Accept/Reject Emails
- [ ] إنشاء `app/Mail/QuoteAcceptedMail.php` — إيميل للمستخدم (المستقل) عند قبول العميل للعرض
- [ ] إنشاء `app/Mail/QuoteRejectedMail.php` — إيميل للمستخدم عند رفض العميل
- [ ] تحديث public portal Accept/Reject endpoints في `QuoteController` لإرسال هذه الإيميلات

#### 26.7 — InvoicePaidMail
- [ ] إنشاء `app/Mail/InvoicePaidMail.php` — EmailTemplate('invoice_paid')، للعميل كإيصال دفع
- [ ] إنشاء `app/Jobs/SendInvoicePaidEmailJob.php`
- [ ] تحديث `InvoiceController::markPaid()` — dispatch `SendInvoicePaidEmailJob` عند تحديد الفاتورة كمدفوعة

#### 26.8 — إضافة Email Channel للإشعارات
- [ ] تحديث `DebtDueSoonNotification` — إضافة `'mail'` لـ `via()` + `toMail()` باستخدام `EmailTemplate::render('debt_due_soon')`
- [ ] تحديث `DebtOverdueNotification` — إضافة `'mail'` + `toMail()`
- [ ] تحديث `FollowUpReminderNotification` — إضافة `'mail'` + `toMail()`
- [ ] تحديث `InvoiceDueSoonNotification` — إضافة `'mail'` + `toMail()`
- [ ] تحديث `InvoiceOverdueNotification` — إضافة `'mail'` + `toMail()`

#### 26.9 — Testing Sprint 2
- [ ] Unit: `QuoteMailTest` — template render, signed URL, variables
- [ ] Unit: `InvoicePaidMailTest` — template render, variables
- [ ] Feature: `QuoteEmailLifecycleTest` — send → accept → email to user
- [ ] Feature: `DebtNotificationEmailTest` — due_soon + overdue channels
- [ ] Feature: `FollowUpEmailTest`

---

### Sprint 3 — Notification Center (مركز الإشعارات)

**الهدف:** تحويل الإشعارات من dropdown بسيط إلى مركز تحكم كامل مع أولويات وتصنيفات وأرشفة.

#### 26.10 — Enums
- [ ] إنشاء `app/Support/Enums/NotificationPriority.php` — Low / Medium / High / Critical + label() + color() + badgeClass()
- [ ] إنشاء `app/Support/Enums/NotificationType.php` — Success / Warning / Error / Info + icon() + bgClass()

#### 26.11 — توحيد بنية بيانات الإشعارات
- [ ] تحديث `toArray()` في جميع Notifications الستة — إضافة: `priority`, `notification_type`, `category`, `related_id`, `related_type`
- [ ] تحديث `AutomationNotification` في CRM Module — إضافة priority keys
- [ ] جدول الأولويات:
...
#### 26.12 — NotificationController
- [ ] تحديث `NotificationController` — إضافة:
  - `index()` — صفحة كاملة مع pagination وفلترة (category, priority, read_status)
  - `markRead(string $id)` — تحديد إشعار كمقروء
  - `markAllRead()` — تحديد الكل كمقروء
  - `archive(string $id)` — أرشفة (حذف ناعم)
  - `destroy(string $id)` — حذف نهائي
- [ ] إضافة Routes في `routes/web.php`

#### 26.13 — Notification Center Page
- [ ] إنشاء `resources/views/notifications/index.blade.php`:
  - تبويبات فلترة: الكل / غير مقروء / حسب الفئة
  - فلتر الأولوية (badge لوني)
  - قائمة إشعارات مع Alpine.js للتفاعل الفوري
  - Per-item actions: قراءة / أرشفة / فتح السجل المرتبط
  - رسالة empty state عند عدم وجود إشعارات

#### 26.14 — Notification Bell Component (تحديث)
- [ ] تحديث `resources/views/components/notification-bell.blade.php`:
  - إضافة Priority color strip لكل إشعار في الـ dropdown
  - Badge لوني حسب الأولوية (أحمر = Critical، برتقالي = High)
  - رابط "عرض الكل" يفتح مركز الإشعارات

#### 26.15 — Notification Components
- [ ] إنشاء `resources/views/components/notification-item.blade.php` — مكوّن صف إشعار واحد قابل للإعادة
- [ ] إنشاء `resources/views/components/notification-widget.blade.php` — ويدجت لوحة التحكم (آخر 5 غير مقروءة + عداد)
- [ ] تكامل `notification-widget.blade.php` في `resources/views/dashboard.blade.php`

#### 26.16 — Testing Sprint 3
- [ ] Unit: `NotificationPriorityEnumTest`
- [ ] Feature: `NotificationCenterTest` — index, markRead, markAllRead, archive, destroy
- [ ] Feature: `NotificationFilterTest` — filter by category, priority, read_status

---

### Sprint 4 — WhatsApp Quick Actions (اختصارات واتساب)

**الهدف:** زر واحد يفتح محادثة واتساب مع رسالة جاهزة. لا API، لا DB، لا تكلفة.

#### 26.17 — WhatsApp Link Generator
- [ ] إنشاء `app/Support/WhatsAppLinkGenerator.php`:
  - `forInvoice(Invoice $invoice): ?string`
  - `forQuote(Quote $quote): ?string`
  - `forClient(Client $client, string $message = ''): ?string`
  - `formatPhone(string $phone): string` — تنظيف الأرقام + إضافة كود الدولة
  - `buildUrl(string $phone, string $message): string`
  - Returns `null` إذا لم يوجد رقم هاتف — لا broken buttons

#### 26.18 — WhatsApp Button Component
- [ ] إنشاء `resources/views/components/whatsapp-button.blade.php`:
  - Props: `url`, `label`, `size` (sm/md)
  - خلفية: `#25D366` (واتساب الرسمي)
  - أيقونة SVG داخلية (بدون CDN خارجي)
  - `target="_blank" rel="noopener noreferrer"`
  - مخفي تلقائياً إذا `$url === null`

#### 26.19 — تكامل الأزرار في Views
- [ ] إضافة `<x-whatsapp-button>` في `resources/views/invoices/show.blade.php`
- [ ] إضافة أيقونة واتساب في قائمة الإجراءات بـ `resources/views/invoices/index.blade.php`
- [ ] إضافة `<x-whatsapp-button>` في `resources/views/quotes/show.blade.php`
- [ ] إضافة أيقونة واتساب في `resources/views/quotes/index.blade.php`
- [ ] إضافة زر "تواصل عبر واتساب" في `resources/views/clients/show.blade.php`
- [ ] إضافة زر واتساب في قائمة الإجراءات بـ `resources/views/clients/index.blade.php`

#### 26.20 — Testing Sprint 4
- [ ] Unit: `WhatsAppLinkGeneratorTest`:
  - `forInvoice_returns_correct_url`
  - `forQuote_returns_correct_url`
  - `forClient_returns_null_when_no_phone`
  - `formatPhone_handles_various_formats`
  - `message_is_properly_url_encoded`
- [ ] Feature: `WhatsAppButtonComponentTest` — renders when url present, hidden when null
...
## ⚙️ Phase 21 — إعدادات النظام من لوحة الإدارة ✅

> التوثيق التفصيلي في: `docs/SETTINGS-ADMIN.md`

### ✅ 21.1 — قاعدة البيانات والنماذج
- [x] Migration: جدول `settings` (key PK, value, group)
- [x] Migration: جدول `email_templates` (key PK, name, subject, body, variables, is_active) مع 3 قوالب افتراضية
- [x] Model `Setting`: get/set/group/setGroup مع Cache تلقائي
- [x] Model `EmailTemplate`: render(key, vars) يستبدل المتغيرات

### ✅ 21.2 — تطبيق الإعدادات على Runtime
- [x] `AppServiceProvider::applyMailSettings()` — يقرأ من DB ويُعيّن `Config::set('mail.*')` في كل request
- [x] Fallback إلى .env إذا فشل DB (أول migrate)
- [x] `CustomResetPasswordNotification` — يستخدم قالب `password_reset` من DB
- [x] `User::sendPasswordResetNotification()` — يستخدم الـ Notification المخصصة
- [x] `WelcomeEmail` — يستخدم قالب `welcome` من DB إذا وُجد

### ✅ 21.3 — Filament Admin Pages
- [x] `MailSettings` Page — إعدادات SMTP كاملة + اختيار Encryption يغيّر Port تلقائياً
- [x] `EmailTemplateResource` — جدول + تعديل (RichEditor) + معاينة (modal) + تجربة (إرسال فوري)
- [x] زر "تجربة الرسالة" في صفحة التعديل وفي جدول القائمة

### ✅ 21.4 — Views
- [x] `emails/template.blade.php` — قالب HTML موحَّد (Header indigo + Body + Footer)
- [x] `filament/pages/mail-settings.blade.php`
- [x] `filament/email-preview.blade.php`

### ✅ 21.5 — إصلاحات البريد الإلكتروني (الإنتاج)
- [x] 500 على forgot-password — `MAIL_SCHEME=smtps` لـ port 465
...
## 🧑‍💼 Phase 24 — ملف العميل المتقدم ✅

### ✅ 24.1 — Health Score Card مفصّلة
- [x] عرض درجة الصحة مع 5 شرائط تقدم ملونة لكل عامل
- [x] العوامل: معدل الدفع (35%) / تكرار العمل (25%) / قيمة الإيراد (20%) / انتظام التواصل (10%) / معدل الاستجابة (10%)
- [x] شارة التقييم (Excellent/Good/Fair/Poor) بجانب الدرجة
- [x] ألوان ديناميكية: أخضر ≥75 / أصفر ≥50 / أحمر <50
- [x] يقرأ `latestHealthScore->factorBreakdown()` من DB

### ✅ 24.2 — Timeline مع تحميل المزيد
- [x] تحميل 15 نشاط بالمرة بدل 50 دفعة واحدة
- [x] زر "تحميل المزيد" يظهر عند وجود بيانات إضافية
- [x] Cursor pagination يدوي عبر `id < last_id`
- [x] `ClientController::timeline()` يعيد `has_more` + `next_cursor`
- [x] Alpine.js `loadTimeline(append)` — false للتحديث، true للتحميل الإضافي

### ✅ 24.3 — تبويب الوسوم الجديد
- [x] تبويب "🏷️ الوسوم" مضاف لشريط التبويبات
- [x] عرض الوسوم الحالية مع زر إزالة لكل وسم (AJAX بدون reload)
- [x] قائمة الوسوم المتاحة للإضافة (غير المُعيَّنة حالياً)
- [x] الاقتراحات الذكية بخلفية أصفر
- [x] `$allTags` مُمرَّر من `ClientController::show()`

### ✅ 24.4 — Bulk Action — تعيين وسم
- [x] `ClientController::bulkAction()` — يدعم archive / tag
- [x] `POST /clients/bulk-action` route مُضاف في `routes/crm.php`

### ⚠️ ملاحظة مهمة
الوسوم الافتراضية تحتاج تشغيل Seeder مرة واحدة:
```bash
