...
**التقييم:** ⭐⭐ يعمل، لكن فيه خطأ حرج بواجهة رسائل الخطأ (رُوجعت حياً 2026-07-08 على `http://127.0.0.1:8000/login` + قراءة الكود: `AuthenticatedSessionController` + `LoginRequest` + `layouts/guest.blade.php`)

**ما يعمل جيداً:**
- RTL عبر `x-guest-layout`
- Rate limiting فعلي (5 محاولات) عبر `RateLimiter` — يعمل بشكل صحيح
- الأخطاء تُعرض دائماً بصرياً (لا يوجد فشل صامت هنا، خلافاً لمشكلة AU4 بالتسجيل)
- "نسيت كلمة المرور" و"تذكّرني" موجودان مع ربط label/for صحيح

**مشاكل:**
| | المشكلة | الأولوية |
|--|---------|---------|
| AU11 | **حرِج:** رسالة فشل الدخول كانت تعرض حرفياً النص `auth.failed` (مفتاح الترجمة الخام) بدل رسالة عربية مفهومة — لأن ملف `lang/ar/auth.php` لم يكن موجوداً إطلاقاً بالمشروع. نفس المشكلة كانت تنطبق على `auth.throttle` | عالية | ✅ تم الإصلاح 2026-07-08 |
| AU12 | **انحدار (regression) ناتج عن إصلاح AU8 بالتسجيل:** بعد تحويل H1 لوحة العلامة التجارية إلى H2، صفحة تسجيل الدخول لم يعد فيها أي `<h1>` | عالية | ✅ تم الإصلاح 2026-07-08 |
| AU5-م | تباين رسائل الخطأ منخفض: `text-red-500` على أبيض = 3.76:1 (نفس مشكلة AU5 بالتسجيل) | عالية | ✅ تم الإصلاح 2026-07-08 |
| AU13 | حقل "كلمة المرور" لا يأخذ `border-red-400` عند فشل الدخول (فقط حقل البريد) | منخفضة | ✅ تم الإصلاح 2026-07-08 |
| AU1 | يستخدم `text-gray-*` بدلاً من `text-slate-*` أو design tokens — نظام ألوان مختلف عن باقي التطبيق | متوسطة | لم يُصلح (يحتاج قرار design tokens) |
| AU2 | Input fields تستخدم `rounded-xl` بدلاً من `rounded-btn` — تناقض بسيط | منخفضة | لم يُصلح |
| AU3 | لا يوجد `focus-visible:` styles صريحة للـ accessibility | منخفضة | لم يُصلح |

**ملفات التعديل (2026-07-08):** `lang/ar/auth.php` (ملف جديد)، `resources/views/auth/login.blade.php` (h1، تباين الألوان، حدّ password). لا Breaking Changes — تم التحقق حياً بمحاولة دخول خاطئة: الرسالة العربية الصحيحة ظهرت، وكلا الحقلين أخذا الحدّ الأحمر، ويوجد H1 واحد ظاهر ("مرحباً بعودتك").
...
**تفاصيل الإصلاحات (AU4-AU8, AU10):** انظر `docs/AUTH-REGISTER-FIXES-2026-07-08.md` — الملفات المعدّلة: `app/Http/Requests/Auth/RegisterRequest.php`، `resources/views/auth/register.blade.php`، `resources/views/layouts/guest.blade.php`. لا Breaking Changes.

**تحديث إضافي (2026-07-08):** حقل "المنطقة الزمنية" أُزيل بالكامل من فورم التسجيل (كان غير مستخدم بأي منطق فعلي — انظر التحليل أعلاه). القيمة تُضبط تلقائياً على `config('app.timezone')` في `RegisterUserAction`، وتبقى قابلة للتعديل من صفحة الإعدادات كما كانت. لا تغيير على جدول `users` أو صفحة الإعدادات.
...
| AU11 | Auth Login | رسالة خطأ الدخول تعرض `auth.failed` حرفياً — ملف `lang/ar/auth.php` غير موجود |
| AU12 | Auth Login | لا يوجد `<h1>` بالصفحة إطلاقاً (انحدار من إصلاح AU8) |
| AU5-م | Auth Login | نفس مشكلة تباين رسائل الخطأ (text-red-500) |
