...
> آخر تحديث: 1 يوليو 2026 | الإصدار: 1.5.0
...
| POST | /invoices/{ulid}/send-client | إرسال بالبريد الإلكتروني |
| GET | /invoices/{ulid}/pdf | تنزيل PDF (mPDF) |
...
| `invoices/show.blade.php` | ورقة فاتورة + أزرار إجراءات | `showSendModal` |
| `invoices/pdf.blade.php` | قالب PDF للطباعة عبر mPDF | — |
...
## ميزات الإرسال (v1.3.0)

### 1. إرسال بالبريد الإلكتروني
- **الزر:** 📧 إرسال للعميل — يفتح Modal للتأكيد
- **الـ Route:** `POST /invoices/{ulid}/send-client`
- **الـ Mail:** `App\Mail\InvoiceMail`
- يُرسَل باسم المستخدم المسجّل مع تفاصيل الفاتورة

### 2. مشاركة عبر واتساب مع PDF
- **الزر:** واتساب + PDF
- **السلوك:**
  1. يُنزَّل ملف PDF للفاتورة تلقائياً على جهاز المستخدم
  2. بعد ثانية يُفتح واتساب مع رسالة جاهزة تحتوي على تفاصيل الفاتورة
  3. يرفق المستخدم الملف يدوياً في المحادثة قبل الإرسال
- **ملاحظة:** واتساب لا يدعم إرفاق ملفات تلقائياً عبر روابط `wa.me`

### 3. تنزيل PDF مباشرة
- **الزر:** PDF (أحمر)
- **الـ Route:** `GET /invoices/{ulid}/pdf`
- **المكتبة:** mPDF مع دعم RTL والعربية
- **القالب:** `resources/views/invoices/pdf.blade.php`
- **اسم الملف:** `invoice-{number}.pdf`

### محتوى رسالة الواتساب
```
مرحباً {اسم العميل}،

يسعدني إشعارك بأن الفاتورة رقم *{رقم الفاتورة}* بمبلغ *{المبلغ} {العملة}* جاهزة.
...
## التحصيل عبر دراهم (v1.5.0 — 1 يوليو 2026)

دفع إلكتروني مباشر من الفاتورة عبر رابط عام (`GET /pay/invoice/{ulid}`) — البوابة تُحصِّل المبلغ نيابة عن المستقل، والتسوية معه يدوية لاحقاً. راجع التوثيق الكامل: **`docs/PAYMENT-COLLECTION.md`**.
...
| الميزة | الأولوية |
|--------|---------|
| ~~دفع إلكتروني مباشر من الفاتورة~~ | ✅ تم — راجع `docs/PAYMENT-COLLECTION.md` |
| واجهة إدارية لتسوية `PaymentCollection` مع المشترك | مستقبلية |
| تفعيل `platform_fee` (عمولة دراهم) | مستقبلية |
