...
> ℹ️ **تحديث**: أُضيف نظام نسخ احتياطي داخل التطبيق (Filament → "النسخ
> الاحتياطية"، `super_admin` فقط) يُكمِّل — ولا يُلغي — سكربتات cron أدناه.
> راجع [BACKUP-SYSTEM.md](BACKUP-SYSTEM.md) للتفاصيل و[RESTORE-RUNBOOK.md](RESTORE-RUNBOOK.md)
> لخطوات الاستعادة، و[DATA-EXPORT.md](DATA-EXPORT.md) لميزة تصدير بيانات
> المستخدم المنفصلة تماماً (من لوحة حساب المستخدم، وليست نسخة نظام).
